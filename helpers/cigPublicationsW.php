<?php 
  ...
  class CigPublicationsWHelper {
    
    public function cigPublicationsWDB ($server = null, $username = null, $password = null, $database = null, $create = false, $autoconnect = true) {
      static $_dba;
      if ((!isset ($_dba) || $create) && ($autoconnect)) {
        ...
        if (isset ($dsn) && $dsn) {
          ...
          if (is_object ($_dba)) {
            ...
            $v = View::getInstance();
            $v->renderError(t('Unable to connect to database.'), t('A database error occured while processing this request.'));
          }
        } else {
          return false;
        }
